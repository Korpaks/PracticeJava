...
public class Book {
    ...
    public static boolean isTitleWord(String word) {
        return switch (word.toLowerCase()) {
            case "a", "an", "the", "and", "but", "for", "nor", "or", "so", "yet", "at", "by", "in", "into", "near", "of", "on", "to", "than", "via" ->
                    true;
            default -> false;
        };
    }
    ...
    public void setAuthor(String author) {
        if(author.isEmpty()){
            ...
            String rightName = "";
            for(String each: arr){
                rightName += capitalize(each)+ " ";
            }
            this.author = rightName.trim();
        }
    }
        ...
        if(!title.trim().contains(" ")){
            ...
            for(String each: arr){
               if(isTitleWord(title)){
                   rightTitle += capitalize(each)+ " ";
               }else {
                   rightTitle += each + " ";
               }
            }
            this.title = rightTitle.substring(0,1).toUpperCase() + rightTitle.substring(1).trim();
        }
